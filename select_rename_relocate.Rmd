...
`num_range()` has the optional arguments `width`, with which we can account for a serie of 0s before the actual number (like if the columns were named `Description001`, `Description002`, ... , we would use a `width` of 3), and `vars`, which works as in `select()`.   
...
Also the optional arguments can utilize `where()`.
