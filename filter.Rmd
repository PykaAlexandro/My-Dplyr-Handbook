...
With `near()`, that accepts only numerical columns instead, we can filter using a chosen tolerance.
...
The default value for the `tol` argument is 0.

```{r}
df %>%
  slice(1:4) %>%
  filter(near(Price, 6.95))
```
